...
Given the data frame created by ProvidersinStateByCounty, this returns the number of providers in a county, the population of the county and the number of providers per 1,000 county residents. 
```{r}
df<-ProviderInStateByCounty("RI", "Primary Care")
SummaryByCounty(df)
```

<strong>DensityMapByCounty</strong>
Given the data frame created by SummaryByCounty and the state name, this plots a heatmap of the provider density in each county in  the state.
```{r}
DensityMapByCounty(df,"Rhode Island")
```
